import { getDBQueryHistory } from '@controllers/servicesController';
...
describe('ServicesRouter', () => {
  ...
  it('GET /api/v1/services/db-health/query-history', async () => {
    const res = await request(app).get('/api/v1/services/db-health/query-history').send({});
    expect(res.status).toBe(200);
